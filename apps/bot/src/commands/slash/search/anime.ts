	...
	public async execute(interaction: ChatInputCommandInteraction<'cached' | 'raw'>) {
					...
					new StringSelectMenuBuilder()
						...
						.setOptions(
							...result.map((data) => ({
								...
								label: cutText(data.title!.userPreferred!, 1e2) ?? 'Unknown Name',
		...
		collector.on('collect', (i) => {
			...
			const section = new TextDisplayBuilder().setContent(
				[
					heading(hyperlink(data.title!.userPreferred!, data.siteUrl!), 2),
					...
					...(data.externalLinks?.filter((item) => item?.type === 'STREAMING')?.length
						? [
								`${bold('Networks:')} ${data.externalLinks
									...
									.map((item) => hyperlink(item.site, item.url as string))
									.join(', ')}`
							]
						: [])
				].join('\n')
...
function getDate(startDate: string, endDate: string | null): string {
	...
	else if (!endDate) return `${startDate} to ?`;
