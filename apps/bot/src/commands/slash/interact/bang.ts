...
export default class extends Command {
	...
	public async execute(interaction: ChatInputCommandInteraction<'cached'>) {
		...
		const response = await axios.get('https://nekos.best/api/v2/shoot').then(({ data }) => data.results[0]);

		const container = new ContainerBuilder()
			.addMediaGalleryComponents(new MediaGalleryBuilder().addItems(new MediaGalleryItemBuilder().setURL(response.url)))
			.addTextDisplayComponents(
				new TextDisplayBuilder().setContent(`${interaction.user.toString()} banged ${member?.toString()}.`)
			)
