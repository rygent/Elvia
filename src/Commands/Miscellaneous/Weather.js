...
module.exports = class extends Command {
	...
	async run(message, args) {
		...
		weather.getAllWeather((err, res) => {
			if (err) {
				return message.reply({ content: 'An API error occurred, Please try again later!' });
			}

			if (res.cod === '404' || !res.sys.country) {
				return message.reply({ content: 'Couldn\'t find that location!' });
			} else if (res.cod === '401') {
				return message.reply({ content: 'Invalid API Key!' });
			}
			...
			const embed = new MessageEmbed()
				.setColor(Color.OPENWEATHER)
				.setAuthor('Open Weather Map', 'https://pbs.twimg.com/profile_images/1173919481082580992/f95OeyEW_400x400.jpg', 'https://openweathermap.org/')
