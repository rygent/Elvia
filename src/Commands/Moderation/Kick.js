...
module.exports = class extends Command {
	...
	async run(message, [target, ...args], data) {
		...
		const m = await message.reply({ content: `Please confirm if you want to kick **${member.displayName}**!`, components: [button] });
		...
		message.channel.awaitMessageComponent({ filter, componentType: 'BUTTON', time: 1000 * 15 }).then(async (interaction) => {
			switch (interaction.customId) {
				case 'confirm': {
					...
					message.guild.members.kick(member, `${message.author.tag}: ${reason}`).then(async () => {
