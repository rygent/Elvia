...
module.exports = class extends Command {
	...
	async run(message, [target]) {
		...
		const result = await axios.get(`https://nekos.life/api/v2/img/cuddle`, { headers }).then(res => res.data);

		const embed = new MessageEmbed()
			.setColor(Color.DEFAULT)
			.setDescription(`<@${message.author.id}> cuddles with ${message.author.id === member.id ? 'themselves' : `<@${member.id}>`}`)
			.setImage(result.url)
