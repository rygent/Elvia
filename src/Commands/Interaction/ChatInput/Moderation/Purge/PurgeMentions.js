...
module.exports = class extends Interaction {
	...
	async run(interaction) {
		...
		fetch.map(m => m).forEach(message => {
			if ((message.mentions.users.first() || message.mentions.members.first() || message.mentions.channels.first() || message.mentions.roles.first()) && !message.pinned) return data.push(message);
		});
