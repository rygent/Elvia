...
module.exports = class extends Interaction {
	...
	async run(interaction) {
		...
		const result = await axios.get(`https://api.waifu.pics/sfw/handhold`, { headers }).then(res => res.data);

		const embed = new MessageEmbed()
			.setColor(Colors.Default)
			.setDescription(`**${interaction.user.username}** holds the hand of **${member.user.username}**.`)
			.setImage(result.url)
