...
module.exports = class extends Interaction {
	...
	async run(interaction) {
		...
		const result = await axios.get(`https://api.waifu.pics/sfw/wave`, { headers }).then(res => res.data);

		const embed = new MessageEmbed()
			.setColor(Colors.Default)
			.setDescription(`**${interaction.user.username}** greets **${member.user.username}**.`)
			.setImage(result.url)
