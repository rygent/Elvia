...
module.exports = class extends Command {
	...
	async run(message, [guildId]) {
		...
		if (guildId) {
			...
			await message.reply({ content: `Attempting to set the Guild Slash Commands in \`${guild.name}\`...` }).then(async (m) => {
			...
			await message.reply({ content: `Attempting to set the Global Slash Commands in \`${this.client.guilds.cache.size.formatNumber()} guilds\`...` }).then(async (m) => {
