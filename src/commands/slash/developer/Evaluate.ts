...
import { AttachmentBuilder, ChatInputCommandInteraction, InteractionCollector, ModalSubmitInteraction } from 'discord.js';
...
export default class extends Command {
	...
	public async execute(interaction: ChatInputCommandInteraction<'cached'>) {
		...
		collector.on('collect', async (i: ModalSubmitInteraction) => {
			...
			try {
				...
				const attachment = new AttachmentBuilder(Buffer.from(this.clean(inspect(evaled, { depth }))))
					.setName('output.txt');

				return void i.editReply({ content: 'Output was too long! The result has been sent as a file.', files: [attachment] });
			} catch (error) {
				const replies = [
	...
	private clean(content: any) {
		...
		const cleaned = content
			...
			.replace(/@/g, `@${String.fromCharCode(8203)}`)
			.replace(new RegExp(this.client.token, 'gi'), this.client.token
				.split('.')
