...
export default class extends Command {
	...
	public async execute(interaction: ChatInputCommandInteraction<'cached' | 'raw'>) {
		...
		const raw = await request(`https://api.themoviedb.org/3/search/tv?api_key=${Credentials.TmdbApiKey}&query=${search}`, {
			method: 'GET',
		...
		collector.on('collect', async (i) => {
			const [ids] = i.values;
			const data = await request(`https://api.themoviedb.org/3/tv/${ids}?api_key=${Credentials.TmdbApiKey}`, {
				method: 'GET',
				headers: { 'User-Agent': Advances.UserAgent },
				maxRedirections: 20
			}).then(({ body }) => body.json());
