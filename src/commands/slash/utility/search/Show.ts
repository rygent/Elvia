...
export default class extends Command {
	...
	public async execute(interaction: ChatInputCommandInteraction<'cached' | 'raw'>) {
		...
		const raw = await request(`https://api.themoviedb.org/3/search/tv?api_key=${Credentials.TmdbApiKey}&query=${search}`, { method: 'GET' });
		...
		collector.on('collect', async (i) => {
			const [ids] = i.values;
			const data = await request(`https://api.themoviedb.org/3/tv/${ids}?api_key=${Credentials.TmdbApiKey}`, { method: 'GET' })
				.then(({ body }) => body.json());
			...
			const embed = new EmbedBuilder()
				...
				.addFields({
					...
					value: [
						...
						...data.first_air_date ? [`${bold(italic('Aired:'))} ${getDate(data.first_air_date, data.last_air_date)}`] : [],
						...data.episode_run_time?.length ? [`${bold(italic('Runtime:'))} ${getRuntime(data.episode_run_time[0])}`] : [],
						...
						...data.networks?.length ? [`${bold(italic('Networks:'))} ${formatArray(data.networks.map(({ name }: any) => name))}`] : []
					].join('\n'),
					inline: false
				})
