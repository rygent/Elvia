	...
	public execute(interaction: ChatInputCommandInteraction<'cached'>) {
		...
		const embed = new EmbedBuilder()
			...
			.setDescription([
				...
				`${bold(italic('Boost Status:'))} ${interaction.guild.premiumSubscriptionCount!} Boosts (${inlineCode(`Level ${interaction.guild.premiumTier}`)})`,
				...
				`${bold(italic('Created:'))} ${time(new Date(interaction.guild.createdTimestamp), 'D')} (${time(new Date(interaction.guild.createdTimestamp), 'R')})`,
				`${bold(italic('Channel:'))} ${formatNumber(channels.filter(channel => channel.isTextBased()).size)} Text / ${formatNumber(channels.filter(channel => channel.isVoiceBased()).size)} Voice`,
				`${bold(italic('Member:'))} ${formatNumber(interaction.guild.memberCount)} members`
			].join('\n'))
			.addFields({ name: underscore(italic(`Roles [${roles.length}]`)), value: `${roles?.length ? formatArray(trimArray(roles, { length: 10 })) : 'None'}`, inline: false })
