...
module.exports = class extends Event {
	...
	async run(message) {
		...
		if (message.content.match(mentionRegex)) {
			return message.reply(`Hello **${message.author.username}**, my prefix on this server is \`${customPrefix}\`.\nUse \`${customPrefix}help\` to get the list of the commands!`);
		}
		...
		if (message.guild) {
			...
			if (afkReason) {
				userData.afk = null;
				await userData.save();
				return message.reply(`**${message.author.username}**, your AFK status has just been deleted!`).then(msg => this.client.setTimeout(() => msg.delete(), 5000));
			}

			message.mentions.users.forEach(async (user) => {
				const dataUser = await this.client.findOrCreateUser({ id: user.id });
				if (dataUser.afk) {
					return message.reply(`**${user.tag}** is currently AFK!\nReason: ${dataUser.afk}`).then(msg => this.client.setTimeout(() => msg.delete(), 15000));
				}
			});
		}
		...
		if (command) {
			...
			if (timestamps.has(message.author.id)) {
				...
				if (now < expirationTime) {
					...
					return message.reply(`You must wait **${timeLeft.toFixed(2)}** second(s) to be able to run the \`${command.name}\` command again!`)
