...
import { type $Dictionary, type $NoInfer, type $SpecialObject, type InternationalizationOptions } from './types.js';
...
export class Internationalization {
	...
	public constructor(options?: InternationalizationOptions) {
		...
		this.languagesDirectory = this.options.defaultLanguageDirectory ?? join(process.cwd(), 'locales');

		const languagePaths = join(this.languagesDirectory, '{{lng}}', '{{ns}}.yaml');

		this.backendOptions = {
			loadPath: this.options.backend?.loadPath ?? languagePaths,
	...
	>(
		...
		if (!this.languagesLoaded) {
			throw new Error('Cannot call this method until Internationalization#init has been called');
		}
